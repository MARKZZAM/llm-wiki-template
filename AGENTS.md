...
- `_compiled-outputs.md`로 이미 처리된 파일을 추적
...
### 처리 대상 식별
`_compiled-sources.md`의 목록과 raw/ 디렉토리를 비교하여
아직 목록에 없는 파일만 처리한다. 매번 전체 raw를 읽지 않는다.
처리가 끝나면 `_compiled-sources.md`에 파일명을 추가한다.
...
#### 처리 대상 식별
`_compiled-outputs.md`의 목록과 output/ 디렉토리를 비교하여
...
6. `_compiled-outputs.md`에 해당 파일명 추가
...
- **고아 문서**: 어느 `_index.md`에도 등록되지 않은 article. 단 `AGENTS.md`, `_template.md`, `_master-index.md`, `_compiled-sources.md`, `_compiled-outputs.md`, 각 `_index.md`는 구조 파일이므로 제외
